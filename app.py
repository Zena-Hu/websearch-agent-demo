...
说明：
    - GUI 直接调用 agent.run_agent(query)，完整链路为：
      query -> agent.need_search() -> 加载 prompt.txt -> tools.web_search()（真实百度千帆 API）-> GUI。
...
# query -> agent.need_search() -> 加载 prompt.txt -> tools.web_search()（真实百度千帆 API）
# ============================================================
    ...
    with st.container():
        ...
        with mid:
            with st.status(f"🔍 正在执行 Agent Pipeline「{q}」...", expanded=True) as status:
                ...
                status.update(label="🌐 正在调用百度千帆 AI 搜索 + LLM（ernie-4.5-turbo-128k）...", state="running")
                try:
    ...
    if not result["success"]:
        ...
        with result_left:
            ...
            with st.container(border=True):
                ...
                st.markdown(
                    '<div class="section-caption">query -&gt; need_search -&gt; prompt.txt -&gt; tools.web_search</div>',
                    unsafe_allow_html=True,
                )
